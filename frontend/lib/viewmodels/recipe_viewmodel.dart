...
class RecipeViewModel with ChangeNotifier {
  ...
  // [?????정] List<String> -> List<Ingredient> ????으??변??
  ...
  // [?????정] getter가 ??닌 ??반 변??로 변??(??계??로직??서 값을 ??당??야 ??????
  ...
  List<Recipe> _calculateFilteredAiRecipes() {
    // [?????정] 기존 getter 로직??'계산 ??수'??변??
    if (_userIngredients.isEmpty) {
      return allAiRecipes;
    }

    print("--- 최종 ??터??검????작 ---");
    // [?????정] List<String> -> List<Ingredient> ????????름??추출
    ...
    for (final recipe in allAiRecipes) {
      ...
      for (final recipeIngredient in recipe.ingredients) {
        // 3. ????장고의 모든 ??료????나????인
        // [?????정] List<String> -> List<Ingredient> ??????ing.name ??용
  ...
  Future<void> fetchRecommendedRecipes() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _filteredAiRecipes = List<Recipe>.from(_recommendedRecipes);
      } else {
  ...
  // Search recipes on server by selected ingredient names (returns fresh list)
  Future<List<Recipe>> searchByIngredientNames(List<String> names) async {
    ...
    if (response.statusCode == 200) {
      ...
      return responseData.map((data) => Recipe.fromJson(data)).toList();
    }
  ...
  void _recalculateAiRecipes() {
    ...
    if (_userIngredients.isEmpty || _allRecipes.isEmpty) {
      _filteredAiRecipes = allAiRecipes; // [?????정] ??료 ??으??AI ??시??'??체'??보여??
      notifyListeners(); // UI 갱신
      return;
    }

    // 2. [?????정] 기존 getter 로직??었??계산 ??수????출
